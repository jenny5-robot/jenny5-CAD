...
{
    color ("black") cylinder_empty(fore_arm_length, fore_arm_shaft_radius, fore_arm_shaft_radius - 2);
}
//---------------------------------------------------------------------------
module fore_arm_bearing_support()
{
    difference(){
      ...
      translate([fore_arm_bearing_support_size[0] - hole_length, fore_arm_bearing_support_size[1] - elbow_pulley_thick / 2, 0] - display_tolerance_z) cube([hole_length, elbow_pulley_thick / 2, fore_arm_bearing_support_size[2]] + 2 * display_tolerance_xyz); 
       ...
       // shaft hole
        translate([fore_arm_bearing_support_size[0] - (elbow_pulley_radius + rb_6906_external_radius + 1.5 - 10), fore_arm_bearing_support_size[1], 0] - display_tolerance_z)
        cylinder (h = fore_arm_bearing_support_size[2] + 2 * display_tolerance, r = rb_6906_external_radius - 2);
        
        // bearing hole
        
         translate([fore_arm_bearing_support_size[0] - (elbow_pulley_radius + rb_6906_external_radius + 1.5 - 10), fore_arm_bearing_support_size[1], 2])
        cylinder (h = rb_6906_thick, r = rb_6906_external_radius, $fn = 80);
...
module fore_arm_bearing_support_with_bearing_holes()
{
    difference(){
     fore_arm_bearing_support();
                // 6906 bearing size
        for (i = [1 : 4])
             translate ([fore_arm_bearing_support_size[0] - (elbow_pulley_radius + rb_6906_external_radius + 1.5 - 10), fore_arm_bearing_support_size[1], 0] - display_tolerance_z)  translate (rbearing_6906_housing_holes_position[i])  cylinder (h = 20, r = 2, $fn = 10); 
}
...
//    translate ([0, - upper_arm_bone_length + upper_arm_bone_top_shift + 2 * rb_626_external_radius , 0]) rotate ([90, 0, 0]) tube_interior_cover(2 * rb_626_external_radius);
    ...
    // 6906 bearing for support
    translate ([0, 0, fore_arm_pulley_thick + 6]) 6906rs();    
    
    translate ([0, 0, fore_arm_pulley_thick + 40 + 6]) 6906rs();
    // gripper
...
// camera support
    translate ([-fore_arm_shaft_radius, c920_depth / 2 + c920_dist_between_holder_holes / 2, fore_arm_length + 10]) rotate([0, 0, -90])  gripper_c920_support_with_camera();
...
{
  translate ([-45, elbow_pulley_radius + rb_6906_external_radius + 1.5, fore_arm_pulley_thick / 2])
   rotate([angle_fore_arm, 0, 0]) 
    ...
    translate ([-elbow_pulley_holes_dist_y - fore_arm_bearing_support_size[2] / 2, elbow_pulley_radius + rb_6906_external_radius + 1.5 - rbearing_6906_housing_size[0] / 2, fore_arm_rotation_motor_support_sheet_size[0] / 2 + 7])
    rotate([0, 90, 0])
    mirror([0, 0, 1])
  ...
  translate ([-elbow_pulley_holes_dist_y - fore_arm_bearing_support_size[2] / 2, fore_arm_bearing_support_size[0] + 10, fore_arm_pulley_thick / 2 + fore_arm_bearing_support_size[1]])
    rotate ([0, 90, 0]) 
        rotate([0, 0, -90]) {
            translate ([0, 0, fore_arm_pulley_thick + 2 + 2 + 5 + 17])
    ...
    // stepper motor
  translate ([ nema_17_with_19_1_gearbox_height - 27,  elbow_pulley_radius + distance_fore_arm_rotation_motor_to_shaft - 1.5, 13 / 2]) 
    rotate ([0, -90, 0])
        fore_arm_rotation_motor()
        ;
    
    // motor pulley
    translate ([-27 - 13 - 5, elbow_pulley_radius + distance_fore_arm_rotation_motor_to_shaft - 1.5, 13 / 2]) rotate ([0, 90, 0]) 
    motor_pulley_6mm_shaft();
    
    // belt
    translate ([-27 - 8, elbow_pulley_radius + distance_fore_arm_rotation_motor_to_shaft - 1.5, 13 / 2]) 
        rotate ([90, 0, 0]) rotate ([0, -90, 0]) 
    belt_on_2_pulleys(9, 35, 63, 6);
...
module elbow_upper_arm_connector()
{
    ...
    distance_between_sheets = 2 * (upper_arm_shaft_radius + tube_bracket_base_thick_strong + 10);
    
    translate([-elbow_sheet_size[0] / 2, -distance_between_sheets / 2,  0]) rotate([90, 0, 0]) elbow_sheet(braket_thick = arm_bracket_thick, tube_radius = upper_arm_shaft_radius);
    translate([-elbow_sheet_size[0] / 2, elbow_sheet_size[2] + distance_between_sheets / 2, 0]) rotate([90, 0, 0]) elbow_sheet(braket_thick = arm_bracket_thick, tube_radius = upper_arm_shaft_radius);
    
    // brakets
    translate([0, 0, + arm_bracket_thick / 2]) rotate([0, 90, 90]) tube_bracket_long_one_hole(bracket_thick = arm_bracket_thick, tube_radius = upper_arm_shaft_radius, half = true, extra_height = 9, hole_y = 20, hole_z = 9);
    translate([0, 0, + arm_bracket_thick / 2]) rotate([0, 90, -90]) tube_bracket_long_one_hole(bracket_thick = arm_bracket_thick, tube_radius = upper_arm_shaft_radius, half = true, extra_height = 9, hole_y = 20, hole_z = 9);
// second set of brackets
    translate([0, 0, - arm_bracket_thick / 2 + elbow_sheet_grip_length]) rotate([0, 90, 90]) tube_bracket_long_one_hole(bracket_thick = arm_bracket_thick, tube_radius = upper_arm_shaft_radius, half = true, extra_height = 9, hole_y = 20, hole_z = 9);
    translate([0, 0, - arm_bracket_thick / 2 + elbow_sheet_grip_length]) rotate([0, 90, -90]) tube_bracket_long_one_hole(bracket_thick = arm_bracket_thick, tube_radius = upper_arm_shaft_radius, half = true, extra_height = 9, hole_y = 20, hole_z = 9);
...
module shoulder(side)
{
    ...
    // motor
    translate ([-side * (- rbearing_6906_housing_size[0] / 2 + distance_upper_arm_rotation_motor_to_shaft), shoulder_plate_size[1] - nema_17_with_19_1_gearbox_height, rb_6906_external_radius + 3 + shoulder_plate_size[2]]) 
        rotate ([-90, 0, 0]) 
            body_arm_motor_with_sheet()
    ;
    
// motor gear for fore_arm  rotation
    translate ([-side * (- rbearing_6906_housing_size[0] / 2 + distance_upper_arm_rotation_motor_to_shaft), shoulder_plate_size[1] + 3 + 14, rb_6906_external_radius + 3 + shoulder_plate_size[2]]) 
    rotate ([90, 0, 0]) 
  ...
  */    
    // bone support 1 top
    translate ([0, first_bearing_shoulder_offset + rbearing_6906_enclosed_housing_size[1] / 2, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick])
      mirror([0, 0, 1])
        rbearing_6906_vertical_housing_bounded_half_with_horizontal_holes()
        ;   
   
       // bone support 1 bottom
    translate ([0, first_bearing_shoulder_offset + rbearing_6906_enclosed_housing_size[1] / 2, shoulder_plate_size[2]])
        rbearing_6906_vertical_housing_bounded_half_with_horizontal_holes()
        ;     
        
// bearing 1        
    translate ([0, rbearing_6906_enclosed_housing_size[1] / 2 + rb_6906_thick / 2 + first_bearing_shoulder_offset, shoulder_plate_size[2] + rb_6906_external_radius + radial_bearing_vertical_housing_base_wall_thick])
rotate([90, 0, 0])
        6906rs();
    
    // bone support 2
    translate ([0, shoulder_plate_size[1] - rbearing_6906_enclosed_housing_size[1] / 2, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick])
      mirror([0, 0, 1])
        rbearing_6906_vertical_housing_bounded_half_with_horizontal_holes();
      
          // bone support 2
    translate ([0, shoulder_plate_size[1] - rbearing_6906_enclosed_housing_size[1] / 2, shoulder_plate_size[2]])
        rbearing_6906_vertical_housing_bounded_half_with_horizontal_holes();
        
        // bearing 2
    translate ([0, -rbearing_6906_enclosed_housing_size[1] / 2 + rb_6906_thick / 2 + shoulder_plate_size[1], shoulder_plate_size[2] + rb_6906_external_radius + radial_bearing_vertical_housing_base_wall_thick])
rotate([90, 0, 0])
        6906rs();
        
        // sheet for supporting the upper arm rotation motor
        translate ([rbearing_6906_housing_size[0] / 2 * side, shoulder_plate_size[1] + 3, rb_6906_external_radius + shoulder_plate_size[2] + 3 - upper_arm_rotation_motor_support_sheet_size[0] / 2]) rotate ([90 * side, 0, 0]) rotate([0, 0, 90 * side]) upper_arm_rotation_motor_support_sheet_top();
...
// brackets for shoulder shaft
 ...
 // sensor support
 // first bearing
 translate ([0, 5, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick + rbearing_688_housing_size[0] / 2 + 3]) rotate([-90, 0, 0]) 688rs();
 // housing
     translate ([0, rbearing_6906_enclosed_housing_size[1] , shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick + rbearing_688_housing_size[0] / 2 + 3])
     rotate([90, 0, 0])
     sensor_bearing_housing_bottom_upper_arm();
    
 // second bearing
 translate ([0, shoulder_plate_size[1] - 20, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick + rbearing_688_housing_size[0] / 2 + 3]) rotate([-90, 0, 0]) 688rs(); 
 
  // housing
     translate ([0, shoulder_plate_size[1], shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick + rbearing_688_housing_size[0] / 2 + 3])//mirror([0, 0, 1])
    rotate([90, 0, 0]) 
     sensor_bearing_housing_top_upper_arm();
     
 // shaft
 
  translate ([0, 0, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick + rbearing_688_housing_size[0] / 2 + 3]) rotate([-90, 0, 0]) cylinder(h = shoulder_plate_size[1], r = 4);
  
  // pulley for sensor shaft rotation
   translate ([0, shoulder_plate_size[1] / 2, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] + radial_bearing_vertical_housing_base_wall_thick + rbearing_688_housing_size[0] / 2 + 3]) rotate([-90, 0, 0]) pulley_upper_arm_sensor_shaft(); 
 
     // sensor pulley for arm shaft - (for sensor)
    translate ([0, shoulder_plate_size[1] / 2, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] - rb_6906_external_radius]) rotate([-90, 0, 0]) pulley_upper_arm_bone_for_sensor();
    
    // belt
    translate ([0, shoulder_plate_size[1] / 2 +3, shoulder_plate_size[2] + rbearing_6906_enclosed_housing_size[2] - rb_6906_external_radius]) rotate ([0, -90, 0]) rotate ([-90, 0, 0]) belt_on_2_pulleys(18, 13, 42, 6);
    
        // continuation with upper arm
    translate ([0, shoulder_plate_size[1] + 3, rb_6906_external_radius + shoulder_plate_size[2] + 3]) rotate ([0, angle_upper_arm, 0]) 
    upper_arm()
    ; 
}
//---------------------------------------------------------------------------
...
module body_articulation(side)
{
    ...
    // motor    
    translate ([rbearing_6905_housing_size[0] / 2 - distance_arm_up_down_motor_to_shaft, -nema_17_with_50_1_gearbox_height + plate_body_size[1], rb_6905_external_radius + plate_body_size[2] + 4])
    rotate ([-90, 0, 0]) nema_17_with_50_1_gearbox();
    
    // motor sheet
    translate ([rbearing_6905_housing_size[0] / 2, plate_body_size[1], - arm_up_down_motor_sheet_size[0] / 2 + rb_6905_external_radius + plate_body_size[2] + 4]) mirror ([0, 1, 0]) rotate ([0, -90, 0]) rotate ([90, 0, 0]) arm_up_down_motor_top_sheet_with_belt_tensioner();

// motor sheet bottom    
    translate ([rbearing_6905_housing_size[0] / 2, 0, - arm_up_down_motor_sheet_size[0] / 2 + rb_6905_external_radius + plate_body_size[2] + 4]) rotate ([0, -90, 0]) rotate ([90, 0, 0]) arm_up_down_motor_bottom_sheet();

    // motor gear
    translate ([- distance_arm_up_down_motor_to_shaft + rbearing_6905_housing_size[0] / 2, plate_body_size[1] + 20, rb_6905_external_radius + plate_body_size[2] + 4])
rotate ([90, 0, 0])  motor_pulley_8mm_shaft();
   ...
   translate ([0, rbearing_6905_vertical_housing_size_bounded_half_small_bottom[1] / 2, rbearing_6905_enclosed_housing_size[2] + plate_body_size[2] + 4])  
   mirror ([0, 0, 1]) 
    rbearing_6905_vertical_housing_bounded_half_with_horizontal_holes();
    ;
   translate ([0, rbearing_6905_vertical_housing_size_bounded_half_small_bottom[1] / 2, plate_body_size[2]])  
    rbearing_6905_vertical_housing_bounded_half_with_horizontal_holes();
    ;
    
    // bearing
    translate ([0, +rbearing_6905_vertical_housing_size_bounded_half_small_bottom[1] / 2 + rb_6905_thick / 2, plate_body_size[2] + rb_6905_external_radius + 4])
        rotate([90, 0, 0]) 6905rs();
    
// second bearing housing
    
   translate ([0, plate_body_size[1] - rbearing_6905_vertical_housing_size_bounded_half_small_bottom[1] / 2, rbearing_6905_enclosed_housing_size[2] + plate_body_size[2] + 4])  
   mirror ([0, 0, 1]) 
    rbearing_6905_vertical_housing_bounded_half_with_horizontal_holes();

   translate ([0, plate_body_size[1] - rbearing_6905_vertical_housing_size_bounded_half_small_bottom[1] / 2, plate_body_size[2]])  
    rbearing_6905_vertical_housing_bounded_half_with_horizontal_holes();

    // bearing
    translate ([0, plate_body_size[1]  - rbearing_6905_vertical_housing_size_bounded_half_small_bottom[1] / 2 + rb_6905_thick / 2, plate_body_size[2] + rb_6905_external_radius + 4])
        rotate([90, 0, 0]) 6905rs();
    
    // shaft
    translate ([0, plate_body_size[1] + pulley_T5_6mm_thick + 2, rb_6905_external_radius + plate_body_size[2] + 4]) 
      rotate ([0, angle_shoulder, 0]) 
        rotate ([90, 0, 0]) 
            color ("black") 
              ...
              ;
    // magnet holder
     translate ([0, plate_body_size[1] + pulley_HTD_5m_thick + 2 - 10, rb_6905_external_radius + plate_body_size[2] + 4]) 
      rotate ([0, angle_shoulder, 0]) 
        rotate ([-90, 0, 0]) 
    ...
    // sensor support
    translate ([-(distance_arm_up_down_motor_to_shaft - nema_17_motor_gearbox_radius - 2 - rbearing_6905_housing_size[0] / 2), plate_body_size[1] + 25, plate_body_size[2] + 4 + rb_6905_external_radius]) mirror ([0, 1, 0]) rotate ([0, -90, 0]) rotate ([-90, 0, 0])
            arm_up_down_sensor_support();    
    
    // pulley 
    translate ([0, plate_body_size[1] + pulley_T5_6mm_thick + 5, rb_6905_external_radius + plate_body_size[2] + 4])
      rotate ([90, 0, 0])
    ...
    translate ([0, -upper_arm_shaft_support_length + plate_body_size[1] + pulley_T5_6mm_thick + 5 + shoulder_plate_size[0] / 2 , rb_6905_external_radius + 4])
      rotate ([0, angle_shoulder, 0])
        translate([offset_shoulder_plate_bracket, 0, shoulder_shaft_radius + plate_body_size[2]])
          rotate ([0, 0, 90])
          shoulder(side) 
            ;
}
...
//translate ([-rbearing_6906_enclosed_housing_size[0] / 2, 0, 0]) 
//fore_arm_bearing_support();
